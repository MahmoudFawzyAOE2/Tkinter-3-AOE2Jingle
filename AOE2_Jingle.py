...
def run(civ_list):
    ...
    # Get the image
    img = ImageTk.PhotoImage(Image.open(r'Pics/{}.jpg'.format(civ)))
    label_img = Label(image=img).grid(row=0, column=0,columnspan=2, padx=70, pady=20)
    print('adding image')

    # Adding Text
    label_text = Label(text='               1               ', font=('Times New Roman', 35, 'bold')).grid(row=1, column=0,columnspan=2, padx=35, pady=20)
    label_text = Label(text='The {}'.format(civ), font=('Times New Roman', 40, 'bold')).grid(row=1, column=0,columnspan=2, padx=35, pady=20)
    print('adding text')
